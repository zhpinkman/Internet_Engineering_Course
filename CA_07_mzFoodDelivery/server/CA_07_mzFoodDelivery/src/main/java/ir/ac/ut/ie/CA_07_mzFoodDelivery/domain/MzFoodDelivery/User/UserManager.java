...
public class UserManager {
  ...
  public UserManager() {
    ...
    //        User user = new User("Ehsan", "Khames", "ekhamespanah@yahoo.com",
    //        "1234","989123456789", new Location(0, 0), 0, 0); try {
  ...
  public void addToCart(CartItem cartItem) throws Exception {
    User user = MzRepository.getInstance().getUser(
  ...
  public String getBriefCartJson() throws SQLException {
    ...
    JsonArray array =
        cartJsonElement.getAsJsonObject().get("cartItems").getAsJsonArray();
    array.forEach(item -> {
      item.getAsJsonObject().addProperty("foodName", item.getAsJsonObject()
                                                         .get("food")
                                                         .getAsJsonObject()
                                                         .get("name")
                                                         .getAsString());
  ...
  public List<CartItem> finalizeOrder() throws Exception {
    ...
    if (foodRepoEmpty(user.getUserCart()))
      throw new Exception(
          "count of this offer is not enough for you to submit");
  ...
  public boolean foodRepoEmpty(List<CartItem> cartItems) throws SQLException {
    for (CartItem cartItem : cartItems) {
      Food food = MzRepository.getInstance().getFood(cartItem.getRestaurantId(),
  ...
  public void deleteFromCart(String restaurantId, String foodName)
      throws Exception {
    User user = MzRepository.getInstance().getUser(
        SecurityContextHolder.getContext().getAuthentication().getName());
  ...
  public List<Order> getOrders() throws SQLException {
    return MzRepository.getInstance().getOrders(
        SecurityContextHolder.getContext().getAuthentication().getName());
  }
